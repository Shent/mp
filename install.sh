...
' |   -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    output ""
      aptitude -y install php7.0-fpm
      aptitude -y install php7.0-opcache php7.0-fpm php7.0 php7.0-common php7.0-gd php7.0-mysql php7.0-imap php7.0-cli php7.0-cgi php-pear php-auth php7.0-mcrypt mcrypt imagemagick libruby php7.0-curl php7.0-intl php7.0-pspell php7.0-recode php7.0-sqlite3 php7.0-tidy php7.0-xmlrpc php7.0-xsl memcached php-memcache php-imagick php-gettext php7.0-zip php7.0-mbstring
      phpenmod mcrypt
      phpenmod mbstring
      ...
      aptitude -y install build-essential libtool autotools-dev automake pkg-config libssl-dev libevent-dev bsdmainutils
    ...
    output ""
    if [[ "$root_email" != "" ]]; then
    echo $root_email >   tee --append ~/.email
    echo $root_email >   tee --append ~/.forward

    if [[ ("$send_email" == "y" || "$send_email" == "Y" || "$send_email" == "") ]]; then
        echo "This is a mail test for the SMTP Service." >   tee --append /tmp/email.message
        echo "You should receive this !" >>   tee --append /tmp/email.message
        echo "" >>   tee --append /tmp/email.message
        echo "Cheers" >>   tee --append /tmp/email.message
          sendmail -s "SMTP Testing" $root_email <   tee --append /tmp/email.message
...
exec bash
' |   -E tee /var/stratum/config/run.sh >/dev/null 2>&1
...
' |   -E tee /etc/nginx/sites-available/$server_name.conf >/dev/null 2>&1
...
' |   -E tee /etc/yiimp/keys.php >/dev/null 2>&1
...
' |   -E tee /var/web/serverconfig.php >/dev/null 2>&1
